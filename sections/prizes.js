  ...
  return (
    <div className="space-y-4">
      ...
      <div className="card bg-white text-gray-800 rounded-lg shadow-md p-4">
        <h3 className="text-lg font-bold mb-3">Weekly High Scores</h3>
          ...
          <tbody>
            {prizes[selectedYear].weeklyHighScores.map((score, index) => (
              <tr key={index} className="border-b">
                <td className="py-2">{score.week}</td>
                <td className="py-2">
      ...
      <div className="card bg-white text-gray-800 rounded-lg shadow-md p-4">
        <h3 className="text-lg font-bold mb-3">Survivor Pool</h3>
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b">
              <th className="text-left py-2">Week</th>
              <th className="text-left py-2">{selectedYear === '2025' && prizes[selectedYear].survivor[11].winner !== undefined ? 'Winner' : 'Eliminated'}</th>
            </tr>
          </thead>
          <tbody>
