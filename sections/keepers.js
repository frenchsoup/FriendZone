  ...
  return (
    <div className="space-y-4">
      ...
      <div className="table-container">
        <div className="card bg-white text-gray-800 rounded-lg shadow-md">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b">
                <th className="text-left py-2 px-2 sm:px-3 w-[20%] sm:w-[15%]">Team</th>
                <th className="text-left py-2 px-2 sm:px-3 w-[20%] sm:w-[15%]">Keeper 1</th>
                <th className="text-right py-2 px-2 sm:px-3 w-[12%] sm:w-[10%]">Draft Cost</th>
                <th className="text-center py-2 px-2 sm:px-3 w-[10%] sm:w-[8%]">Tag</th>
                <th className="text-right py-2 px-2 sm:px-3 w-[10%] sm:w-[8%]">Cost</th>
                <th className="text-left py-2 px-2 sm:px-3 w-[20%] sm:w-[15%]">Keeper 2</th>
                <th className="text-right py-2 px-2 sm:px-3 w-[12%] sm:w-[10%]">Draft Cost</th>
                <th className="text-center py-2 px-2 sm:px-3 w-[10%] sm:w-[8%]">Tag</th>
                <th className="text-right py-2 px-2 sm:px-3 w-[10%] sm:w-[8%]">Cost</th>
                <th className="text-right py-2 px-2 sm:px-3 w-[10%] sm:w-[8%]">Remaining</th>
                {!locks[selectedYear] && <th className="text-right py-2 px-2 sm:px-3 w-[8%] sm:w-[10%]">Actions</th>}
              </tr>
            </thead>
            <tbody>
              {keepers[selectedYear].map((team, index) => {
                ...
                return (
                  <tr key={index} className={`border-b ${index % 2 === 0 ? 'table-row-even' : 'table-row-odd'}`}>
                    ...
                    <td className="text-right py-2 px-2 sm:px-3">${displayTeam.cost1 || 0}</td>
                    <td className="py-2 px-2 sm:px-3">
                      {!locks[selectedYear] ? (
                        <input
                          type="text"
                    ...
                    <td className="text-right py-2 px-2 sm:px-3">${displayTeam.cost2 || 0}</td>
                    <td className="text-right py-2 px-2 sm:px-3">${displayTeam.remaining || 200}</td>
                    {!locks[selectedYear] && (
                      <td className="text-right py-2 px-2 sm:px-3">
                        <button
